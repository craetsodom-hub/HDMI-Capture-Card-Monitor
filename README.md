...
Audio monitoring is opt-in: the customer selects a capture endpoint or leaves `No audio` selected, chooses an explicit output or `System default output`, and controls local mute and volume. It uses an event-driven shared-mode WASAPI worker, a bounded complete-frame ring buffer, and independent cleanup from the video session. Current laptop testing proved endpoint discovery, muted capture/render operation, deterministic release, and repeated lifecycle cleanup, but recurring underruns remain a Phase 5 blocker. Audible headphone quality, independent-clock drift, A/V synchronization, and physical USB HDMI audio remain unvalidated.
