...
Audio monitoring is opt-in: the customer selects a capture endpoint or leaves `No audio` selected, chooses an explicit output or `System default output`, and controls local mute and volume. It uses an event-driven shared-mode WASAPI worker, a bounded complete-frame ring buffer, and independent cleanup from the video session. A corrected ten-minute muted laptop run completed with zero underruns, overruns, or dropped frames, and twenty repeated lifecycle cycles released both endpoints. Audible headphone quality, HDMI A/V synchronization, and physical USB HDMI audio remain unvalidated.
